...
def run_backtest_with_tax(
    ...
    # Initialize capital loss carryforward
    ...
    for i, fy in enumerate(fys):
        ...
        # Apply any capital loss carryforward from previous year
        if capital_loss_carryforward > 0:
            print(f"Applying capital loss carryforward of {capital_loss_carryforward} AUD from previous year")
            # Add the capital loss to this year's deductions
            year_deductions = deductions_series.loc[year] + capital_loss_carryforward
            deductions_series.loc[year] = year_deductions
        else:
        ...
        # Add dates to pf.orders.records
            
        ords = pd.DataFrame(pf.orders.records)
        ords['date'] = year_price.index[ords['idx']]
        all_orders = pd.concat([all_orders, ords], axis = 0)
        ...
        # Calculate trading profit/loss for this year
        if not trades_df.empty:
            year_cgt_amount = trades_df["cgt_amount_(AUD)"].sum()
            # If negative, set as capital loss carryforward for next year
            if year_cgt_amount < 0:
                print(f"Net capital loss of {abs(year_cgt_amount)} AUD detected for year {year}")
                # Save the negative value (absolute loss amount) for next year
                next_year_capital_loss = abs(year_cgt_amount)
                # For current year tax calculation, set CGT to zero (can't claim negative CGT)
                trades_df["cgt_amount_(AUD)"] = trades_df["cgt_amount_(AUD)"].clip(lower=0)
            else:
                next_year_capital_loss = 0
        else:
            next_year_capital_loss = 0
        ...
        cgt_tax_usd = tax_result['cgt_tax_(aud)'] / year_aud[-1]
        ...
        tax_adjusted_values.iloc[-1] -= cgt_tax_usd
        portfolio_values_taxed_by_year[year] = tax_adjusted_values
        print(f"Paid total tax for {year} of {tax_result['total_tax_aud']} in AUD of which {cgt_tax_payments[year]} was CGT tax, also (AUD).\n\
              Portfolio value is measured in USD and CGT tax payment in USD was: {cgt_tax_usd} USD. This was subtracted from the portfolio value.")
        
        # Handle year-end position and tax payment
        if end_cash >= cgt_tax_usd:
            # If we have enough cash, just deduct the tax
            current_cash = end_cash - cgt_tax_usd
        ...
        print(f"\nCurrent cash at end of year loop: {current_cash}, current assets: {current_assets}")
        all_trades = pd.concat([all_trades, trades_df], axis = 0)
        
        # Update capital loss carryforward for next year
        capital_loss_carryforward = next_year_capital_loss
        if capital_loss_carryforward > 0:
            print(f"Capital loss of {capital_loss_carryforward} AUD will be carried forward to next year")

        # If we have assets and there's a next year, add a buy order at start of next year
        if i < len(fys) - 1 and current_assets > 0.000000001:  # Add tolerance check to avoid tiny asset values
            ...
            print(f"\nTarget percent for day 1 of year {year} order: {target_percent}")
            ...
            order_at_start_of_year = np.nan
            print(f"No assets at end of year {year}, no order will be placed at start of next FY")
